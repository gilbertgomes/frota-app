...
export default {
    ...
    data() {
        return {
            ...
            headersitem: [{
                ...
                { text: 'Tipo Item', value: 'tipoitem', class: "cyan dark 1 white--text" },
                ...
                { text: 'Dt Inicio', value: 'dtinicio', class: "cyan dark 1 white--text" },
            ...
            lintaitens: [],   
            overlay: false,   
            formulario: 'Lista de Itens da OS',
    ...
    methods: {
        exibeitens(item) {
            const  urldadosositem = process.env.VUE_APP_HOST + "ositem/search/" + item
            this.overlay = true
            this.itensprojeto = item.id
            this.axios.get(urldadosositem)
            .then(response => {         
        ...
        excluiritens(item) {
            const  key = 'id90'
            const  urldadoexcluiritem = process.env.VUE_APP_HOST + "ositem/alt/" + key
            this.ositem.id = item.ositem
            this.overlay = true
            this.axios.post(urldadoexcluiritem, this.ositem)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregalista', (os) => {
            localStorage.os = os.id
            this.exibeitens(os.id)
            this.dialog = true
            this.alert  = true
        })
    },   
