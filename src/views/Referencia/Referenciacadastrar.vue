                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Tipo de referencia" v-model="referencia.referencia" :rules="referencia.freferenciaRules" name="referencia" :value="referencia.referencia"  @input="textToUpper('referencia')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                           
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            referencia: {
              id: 0,
              referencia: '',
              freferenciaRules: [ v => !!v || 'Tipo de referencia é obrigatório!'],               
              visualiza: false,
            },
            msgadd: 'Cadastro execultado com sucesso0',
            nomeFormulario: 'Referência'
        }
    },
    methods: {
        insert() {
            const  urlinsertreferencia = process.env.VUE_APP_HOST  + "referencia/add/" + 1
            
            this.axios.post(urlinsertreferencia, this.referencia)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'referencia'){
                this.referencia.referencia = this.referencia.referencia.toUpperCase()
            }                
        },
        fechaFomulariomsg(){
            this.dialog  = false
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregacadastro', (referencia) => {
            localStorage.visualiza = referencia.visualiza
            this.dialog = true
            this.alert = true
