...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/Referencia/Referenciacadastrar.vue'),
        Alterar:   () => import('@/views/Referencia/Referenciaalterar.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Referência', value: 'referencia', class: "cyan dark 1 white--text" },
            ...
            nomeformulario: 'Referencia'
        }
    },
    methods: {
        carregaForm() {
            this.referencia.visualiza = true
            EventBus.$emit('carregacadastro', this.referencia)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadosreferencia = process.env.VUE_APP_HOST + "referencia/search/" + key

            this.axios.get(urldadosreferencia)
            .then(response => {           
        ...
        alteraritem(item) {
            this.referencia.id = item.id
            this.referencia.referencia = item.referencia
            this.referencia.visualiza = true
            EventBus.$emit('carregaalteracao', this.referencia)
            this.initialize()
        },
    },
