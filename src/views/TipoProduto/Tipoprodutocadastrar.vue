                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Tipo de Produto" v-model="tipoproduto.tipoproduto" :rules="tipoproduto.tipoprodutoeRules" name="tipoproduto" :value="tipoproduto.tipoproduto"  @input="textToUpper('tipoproduto')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                           
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            tipoproduto: {
              id: 0,
              tipoproduto: '',
              tipoprodutoeRules: [ v => !!v || 'Tipo Produto é obrigatório!'],               
              visualiza: false,
            },
            msgadd: 'Cadastro execultado com sucesso0',
            nomeFormulario: 'Tipo Produto'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlinserttipoproduto = process.env.VUE_APP_HOST  + "tipoproduto/add/" + key
            
            this.axios.post(urlinserttipoproduto, this.tipoproduto)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'tipoproduto'){
                this.tipoproduto.tipoproduto = this.tipoproduto.tipoproduto.toUpperCase()
            }                
        },
        fechaFomulariomsg(){
            this.dialog  = false
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregacadastro', (tipoproduto) => {
            localStorage.visualiza = tipoproduto.visualiza
            this.dialog = true
            this.alert = true
