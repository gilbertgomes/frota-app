...
export default {
    ...
    data() {
        return {
            ...
            nomeFormulario: 'Alterar Tipo Receita / Despesa'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlupdatetiporecdesp = process.env.VUE_APP_HOST  + "recedesp/alt/" + key
            
            this.axios.post(urlupdatetiporecdesp, this.tiporecdesp)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregaalteracao', (tiporecdesp) => {
            localStorage.visualiza = tiporecdesp.visualiza
            localStorage.id = tiporecdesp.id
            localStorage.tipodr = tiporecdesp.tipodr
            this.tiporecdesp.id = localStorage.id
            this.tiporecdesp.tipodr =  localStorage.tipodr
