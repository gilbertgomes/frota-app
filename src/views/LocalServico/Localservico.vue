...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/LocalServico/Localservicocadstro.vue'),
        Alterar:   () => import('@/views/LocalServico/Localservicoalterar.vue'),
    },
    name: 'Local Servico',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Local', value: 'localservico', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.local.visualiza = true
            EventBus.$emit('carregacadastro', this.local)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            this.overlay = true
            const  urldadoslocal = process.env.VUE_APP_HOST + "localexecucao/search/" + key

            this.axios.get(urldadoslocal)
            .then(response => {           
        ...
        alteraritem(item) {
            this.local.id = item.id
            this.local.local = item.localservico
            this.local.visualiza = true
            EventBus.$emit('carregaalteracao', this.local)
            this.initialize()
        },
    },
    watch: {
