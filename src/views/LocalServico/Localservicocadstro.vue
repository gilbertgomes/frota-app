                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Local do Serviço" v-model="local.local" :rules="local.localRules" name="local" :value="local.local"  @input="textToUpper('local')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                           
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            local: {
              id: 0,
              local: '',
              localRules: [ v => !!v || 'Nome do Local Serviço é obrigatório!'],               
              visualiza: false,
            },
            msgadd: 'Cadastro execultado com sucesso0',
            nomeFormulario: 'Local Serviço'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlinsertlocal = process.env.VUE_APP_HOST  + "localexecucao/add/" + key
            
            this.axios.post(urlinsertlocal, this.local)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregacadastro', (local) => {
