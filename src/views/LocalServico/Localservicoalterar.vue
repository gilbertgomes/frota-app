                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Local Serviço" v-model="local.local" :rules="local.localRules" name="local" :value="local.local"  @input="textToUpper('local')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                           
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            local: {
              id: 0,
              local: '',
              localRules: [ v => !!v || 'Local do Serviço é obrigatório!'],               
              visualiza: false,
            },
            msgadd: 'Alteração execultada com sucesso!',
            nomeFormulario: 'Alterar Local Serviço'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlupdatelocal = process.env.VUE_APP_HOST  + "localexecucao/alt/" + key
            
            this.axios.post(urlupdatelocal, this.local)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'local'){
                this.local.local = this.local.local.toUpperCase()
            }                
        },
        fechaFomulariomsg(){
            this.dialog  = false
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregaalteracao', (local) => {
            localStorage.visualiza = local.visualiza
            localStorage.id = local.id
            localStorage.local = local.local
            this.local.id = localStorage.id
            this.local.local = localStorage.local
