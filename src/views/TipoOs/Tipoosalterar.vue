                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Tipo de OS" v-model="tipoos.tipoos" :rules="tipoos.tipoosRules" name="tipoos" :value="tipoos.tipoos"  @input="textToUpper('tipoos')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                        
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            nomeFormulario: 'Alterar Tipo OS'
        }
    },
    methods: {
        update() {
            const  key = 'frota2021house'
            const  urlupdatetipoos = process.env.VUE_APP_HOST  + "tipoos/alt/" + key
            
            this.axios.post(urlupdatetipoos, this.tipoos)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'tipoos'){
                this.tipoos.tipoos = this.tipoos.tipoos.toUpperCase()
            }                
        },
        fechaFomulariomsg(){
            this.dialog  = false
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregaalteracao', (tipoos) => {
            localStorage.visualiza = tipoos.visualiza
            localStorage.id         = tipoos.id
            localStorage.tipoos = tipoos.tipoos
            this.tipoos.id = localStorage.id
            this.tipoos.tipoos =  localStorage.tipoos
