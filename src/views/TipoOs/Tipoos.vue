...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/TipoOs/Tipooscadastrar.vue'),
        Alterar:   () => import('@/views/TipoOs/Tipoosalterar.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
    ...
    methods: {
        carregaForm() {
            this.tipoos.visualiza = true
            EventBus.$emit('carregacadastro', this.tipoos)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadostipoos = process.env.VUE_APP_HOST + "tipoos/search/" + key

            this.axios.get(urldadostipoos)
            .then(response => {           
        ...
        alteraritem(item) {
            this.tipoos.id = item.id
            this.tipoos.tipoos = item.tipoos
            this.tipoos.visualiza = true
            EventBus.$emit('carregaalteracao', this.tipoos)
            this.initialize()
        },
    },
