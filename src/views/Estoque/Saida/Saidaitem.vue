                ...
                <v-card-text> 
                    <div class="text-center">  
                        <v-alert v-model="alert" type="cyan">
                            Cadastro {{this.nomeFormulario}} da Saida Nº {{this.saidaitem.saida}} 
                        </v-alert>
                    </div>
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Descrição do Produto" v-model="saidaitem.descricaoproduto"  @click="carregapesquisaproduto()" :rules="saidaitem.descricaoproduto" name="descricaoproduto" :value="saidaitem.descricaoproduto"  @input="textToUpper('descricaoproduto')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex> 
                            <v-flex xs12 sm64 md4>
                                <v-text-field  label="Unidade do Produto" v-model="saidaitem.descunidade" @click="carregapesquisaunidade()" :rules="saidaitem.qdeRules" name="qde" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm64 md4>
                                <v-text-field  label="Referencia do Produto" v-model="saidaitem.descreferencia" @click="carregapesquisareferencia()" :rules="saidaitem.referenciaRules" name="descreferencia" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                           <v-flex xs12 sm64 md4>
                                <v-text-field  label="Quantidade do Produto" v-model="saidaitem.qde" :rules="saidaitem.qdeRules"   name="qde" :value="saidaitem.qde"  @input="somentenumero('qde')"  clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm64 md4>
                                <v-text-field  label="Valor Unitário" v-model="saidaitem.valorun" :rules="saidaitem.valorunRules"   name="valorun"  :value="saidaitem.valorun"  @input="somentenumero('valorun')"  clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm64 md4>
                                <v-text-field  label="Valor Venda" v-model="saidaitem.valorvenda" :rules="saidaitem.valorvendatRules"  name="valorvenda"  clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm64 md4>
                                <v-text-field  label="Valor Total" v-model="saidaitem.valortot" :rules="saidaitem.valortotRules"  name="valortot"   :value="saidaitem.valortot"  @input="somentenumero('valortot')"  clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm66 md6>
                                <v-text-field  label="Nº Documento" v-model="saidaitem.documento" :rules="saidaitem.documentoRules"  name="documento"  clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm66 md6>
                                <v-text-field  label="Destino da Saída" v-model="saidaitem.destino" :rules="saidaitem.destinoRules" name="destino"  :value="saidaitem.destino"  @input="textToUpper('destino')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Observação" v-model="saidaitem.obs" :rules="saidaitem.obstRules" name="obs" :value="saidaitem.obs"  @input="textToUpper('obs')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                            
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    methods: {
        insert() {
            ...
            const  urlinsertsaidaitem = process.env.VUE_APP_HOST  + "saidaitem/add/" + key
            
            this.axios.post(urlinsertsaidaitem, this.saidaitem)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'obs'){
                this.saidaitem.obs = this.saidaitem.obs.toUpperCase()
            } else if(id == 'obs'){
                this.saidaitem.obs = this.saidaitem.obs.toUpperCase()
            }                 
        },
        ...
        pegaitemproduto(item){
            this.saidaitem.produto = item.id
            this.saidaitem.descricaoproduto = item.produto  
            this.alert3  = false 
            this.dialog3 = false               
        },
        ...
        pegaitemunidade(item){     
            this.saidaitem.unidade = item.id
            this.saidaitem.descunidade = item.unidade  
            this.alert4  = false 
            this.dialog4 = false               
        },
        ...
        pegaitemreferencia(item){     
            this.saidaitem.referencia = item.id
            this.saidaitem.descreferencia = item.referencia  
            this.alert5 = false 
            this.dialog5 = false               
        },
        fechar(){             
        ...
        somentenumero(campo){
            if (campo == 'qde'){
                ...
                this.saidaitem.qde = this.saidaitem.qde.replace(",", ".")
            } else if (campo == 'valorun'){
                this.saidaitem.valorun =  this.saidaitem.valorun.replace(",", ".") 
                this.saidaitem.valortot =  this.calculasoma(this.saidaitem.qde, this.saidaitem.valorun )
            } else if (campo == 'valortot'){
                this.saidaitem.valortot = this.saidaitem.valortot.replace(",", ".") 
            }
        },
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregaitem', (saida) => {
            localStorage.visualiza = saida.visualiza
            localStorage.saida = saida.id
            this.saidaitem.saida = localStorage.saida
            this.dialog = true
            this.alert = true
