                    ...
                    <v-card-text>                     
                        <v-row>  
                            <v-flex xs12 sm612 md12>
                                <v-text-field  label="Tipo de Condutor Veiculo" v-model="tipocondutor.tipocondutor" :rules="tipocondutor.tipocondutoreRules" name="tipocondutor" :value="tipocondutor.tipocondutor"  @input="textToUpper('tipocondutor')" clearable placeholder="Dense & Rounded" filled rounded dense></v-text-field>     
                            </v-flex>                           
                        </v-row>
                    </v-card-text>
                    <div class="text-center"> 
...
export default {
    ...
    data() {
        return {
            ...
            tipocondutor: {
              id: 0,
              tipocondutor: '',
              tipocondutoreRules: [ v => !!v || 'Tipo Motorista é obrigatório!'],               
              visualiza: false,
            },
            msgadd: 'Cadastro execultado com sucesso0',
            nomeFormulario: 'Tipo Motorista'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlinserttipomotorista = process.env.VUE_APP_HOST  + "tipocondutor/add/" + key
            
            this.axios.post(urlinserttipomotorista, this.tipocondutor)
            .then(response => {           
        ...
        textToUpper(id) {
            if(id == 'tipocondutor'){
                this.tipocondutor.tipocondutor = this.tipocondutor.tipocondutor.toUpperCase()
            }                
        },
        fechaFomulariomsg(){
            this.dialog  = false
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregacadastro', (tipocondutor) => {
            localStorage.visualiza = tipocondutor.visualiza
            this.dialog = true
            this.alert = true
