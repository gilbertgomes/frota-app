...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/SituacaoMotorista/Situacaomotoristacadastrar.vue'),
        Alterar:   () => import('@/views/SituacaoMotorista/Situacaomotoristaalterar.vue'),
    },
    ...
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Situação Motorista', value: 'situacaocondutor', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.situacaocondutor.visualiza = true
            EventBus.$emit('carregacadastro', this.situacaocondutor)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadossituacaomotorista = process.env.VUE_APP_HOST + "situacaocondutor/search/" + key

            this.axios.get(urldadossituacaomotorista)
            .then(response => {           
        ...
        alteraritem(item) {
            this.situacaocondutor.id = item.id
            this.situacaocondutor.situacaocondutor = item.situacaocondutor
            this.situacaocondutor.visualiza = true
            EventBus.$emit('carregaalteracao', this.situacaocondutor)
            this.initialize()
        },
    },
