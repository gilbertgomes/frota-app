 ...
 <v-card>
    ...
    <template v-slot:top>
      ...
      <v-spacer></v-spacer> 
        ...
        <template v-slot:item.numerooc="{ item }">
        <v-chip
            color="teal dark"
            dark
        >
      ...
      <v-tooltip left color="blue">
        <template v-slot:activator="{ on, attrs }">
          ...
          <v-btn class="smf-gradient" fab dark small color="#0064b7" v-bind="attrs" v-on="on">
              <!-- opcao para carregar formulario modal e atulizar o GRID-->
              <v-icon  @click="imprimeitem(item)">
                  mdi-printer
              </v-icon>
          </v-btn>
          <v-btn class="smf-gradient" fab dark small color="#c2185b" v-bind="attrs" v-on="on">
              <!-- opcao para carregar formulario modal e atulizar o GRID-->
              <v-icon  @click="exibeexcluiitem(item)">
                 mdi-delete-forever-outline
              </v-icon>
          </v-btn> 
          <v-btn class="smf-gradient" fab dark small color="#5c007a" v-bind="attrs" v-on="on">
              <!-- opcao para carregar formulario modal e atulizar o GRID-->
              <v-icon  @click="inseriitem(item)">
                  mdi-menu
              </v-icon>
          </v-btn>
        </template>
...
export default {
    components: {   
        ...
        Cadastrar:    () => import('@/views/OC/Occadastrar.vue'),
        Alterar:      () => import('@/views/OC/Ocalterar.vue'),
        CadastarItem: () => import('@/views/OC/Ocitem.vue'),
        Excluiritem: () => import('@/views/OC/Ocitemlistaexclusao.vue'),
        Impressão: () => import('@/views/OC/Ocitemimpressao.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Numero OC', value: 'numerooc', class: "cyan dark 1 white--text" },
                { text: 'Solicitante', value: 'solicitante', class: "cyan dark 1 white--text" },
                { text: 'Setor', value: 'setor', class: "cyan dark 1 white--text" },
                { text: 'Fonte Pag', value: 'fontepagadora', class: "cyan dark 1 white--text" },
    ...
    methods: {
        ...
        inseriitem(item) {
            this.oc.id = item.id
            this.oc.visualiza = true
            EventBus.$emit('carregaitemoc', this.oc)
            this.initialize()
        },
        exibeexcluiitem(item) {
            this.oc.id = item.id
            this.oc.visualiza = true
