...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/TipoVeiculo/Tipoveiculocadastrar.vue'),
        Alterar:   () => import('@/views/TipoVeiculo/Tipoveiculoalterar.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Tipo Veículo', value: 'tipoveiculo', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.tipoveiculo.visualiza = true
            EventBus.$emit('carregacadastro', this.tipoveiculo)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadosfabricante = process.env.VUE_APP_HOST + "tipoveiculo/search/" + key
        ...
        alteraritem(item) {
            this.tipoveiculo.id = item.id
            this.tipoveiculo.tipoveiculo = item.tipoveiculo
            this.tipoveiculo.visualiza = true
            EventBus.$emit('carregaalteracao', this.tipoveiculo)
            this.initialize()
        },
    },
