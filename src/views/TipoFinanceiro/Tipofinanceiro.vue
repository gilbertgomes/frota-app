...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/TipoFinanceiro/Tipofinanceirocadastrar.vue'),
        Alterar:   () => import('@/views/TipoFinanceiro/Tipofinanceiroalterar.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Tipo Financeiro', value: 'tipofinanceiro', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            ...
            EventBus.$emit('carregacadastro', this.tipofinanceiro)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadostipofinanceiro = process.env.VUE_APP_HOST + "tipofinanceiro/search/" + key

            this.axios.get(urldadostipofinanceiro)
            .then(response => {           
