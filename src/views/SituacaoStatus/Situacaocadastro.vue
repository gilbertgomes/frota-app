...
export default {
    ...
    data() {
        return {
            ...
            nomeFormulario: 'Situação / Status'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlinsertsituacao = process.env.VUE_APP_HOST  + "situacao/add/" + key
            
            this.axios.post(urlinsertsituacao, this.situacao)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregacadastro', (situacao) => {
