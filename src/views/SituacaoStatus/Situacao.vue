...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/SituacaoStatus/Situacaocadastro.vue'),
        Alterar:   () => import('@/views/SituacaoStatus/Situacaoalterar.vue'),
    },
    name: 'Fabricante',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Situação / Status', value: 'situacao', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.situacao.visualiza = true
            EventBus.$emit('carregacadastro', this.situacao)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            this.overlay = true
            const  urldadossituacao = process.env.VUE_APP_HOST + "situacao/search/" + key

            this.axios.get(urldadossituacao)
            .then(response => {           
        ...
        alteraritem(item) {
            this.situacao.id = item.id
            this.situacao.situacao = item.situacao
            this.situacao.visualiza = true
            EventBus.$emit('carregaalteracao', this.situacao)
            this.initialize()
        },
    },
    watch: {
