...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/TipoRota/Tiporotacadastrar.vue'),
        Alterar:   () => import('@/views/TipoRota/Tiporotaalterar.vue'),
    },
    name: 'TipoRota',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Tipo Rota', value: 'tiporota', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.tiporota.visualiza = true
            EventBus.$emit('carregacadastro', this.tiporota)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadostiporota = process.env.VUE_APP_HOST + "tiporota/search/" + key

            this.axios.get(urldadostiporota)
            .then(response => {           
        ...
        alteraritem(item) {
            this.tiporota.id = item.id
            this.tiporota.tiporota = item.tiporota
            this.tiporota.visualiza = true
            EventBus.$emit('carregaalteracao', this.tiporota)
            this.initialize()
        },
    },
