...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/FormaPagamento/Formapagamentoalterar.vue'),
        Alterar:   () => import('@/views/FormaPagamento/Formapagamentocadastrar.vue'),
    },
    name: 'Cliente',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Forma Pagamento', value: 'formapag', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.formapagamento.visualiza = true
            EventBus.$emit('carregacadastro', this.formapagamento)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            this.overlay = true
            const  urldadosformapagamento = process.env.VUE_APP_HOST + "formapagamento/search/" + key

            this.axios.get(urldadosformapagamento)
            .then(response => {           
        ...
        alteraritem(item) {
            ...
            this.formapagamento.formapag = item.formapag
            this.formapagamento.visualiza = true
            EventBus.$emit('carregaalteracao', this.formapagamento)
            this.initialize()
        },
    },
    watch: {
