...
export default {
    ...
    data() {
        return {
            ...
            nomeFormulario: 'Acessório'
        }
    },
    methods: {
        insert() {
            const  key = 'frota2021house'
            const  urlinsertacessorio = process.env.VUE_APP_HOST  + "acessorio/add/" + key
            
            this.axios.post(urlinsertacessorio, this.acessorio)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregaitem', (veiculo) => {
            localStorage.visualiza = veiculo.visualiza
            localStorage.id = veiculo.id
            localStorage.placa = veiculo.placa
            localStorage.carro = veiculo.veiculo
            this.acessorio.veiculo = localStorage.id
